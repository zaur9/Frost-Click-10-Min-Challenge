...
import { refreshBattleTotalsDOM, openBattleLeaderboardModal } from './leaderboard';
import {
...
export function WalletEffects() {
  ...
  useEffect(() => {
    const btn = document.getElementById('start-show-leaderboard');
    const onLb = () => void openBattleLeaderboardModal();
    btn?.addEventListener('click', onLb);
    return () => btn?.removeEventListener('click', onLb);
  }, []);
