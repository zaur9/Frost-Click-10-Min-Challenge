...
import { refreshBattleTotalsDOM } from '../battle/leaderboard';
import {
...
export function WalletEffects() {
  ...
  useEffect(() => {
    ...
    const runWhenIdle = () => {
      if (cancelled || isGameActive()) return;
      void refreshBattleTotalsDOM();
    };

    let cancelScheduled: (() => void) | undefined;
    if (typeof window.requestIdleCallback === 'function') {
      const id = window.requestIdleCallback(runWhenIdle, { timeout: 1200 });
      cancelScheduled = () => window.cancelIdleCallback(id);
    } else {
      const id = window.setTimeout(runWhenIdle, 500);
